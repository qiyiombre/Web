...
这个项目不只是一个前端可视化页面。它包含完整的前后端分离结构、用户认证、REST API、PostgreSQL 云数据库持久化、AI 辅助分析、缓存和离线草稿能力，可以作为 Web 技术，尤其是 Web 后端技术的课程项目进行展示。
...
- 数据持久化：使用 Supabase PostgreSQL 保存用户、会话、星云图、日志、标签和 AI 缓存。
...
| 后端 | Node.js, Express 5, node-postgres |
| 认证 | Cookie Session, HttpOnly Cookie, scrypt 密码哈希 |
| 数据库 | PostgreSQL, 外键约束, 索引, 多表关系 |
...
| 数据库访问 | `server/src/db.js` | 初始化 PostgreSQL 表结构，封装用户、会话、星云图、日志、标签、关系边和缓存操作 |
...
项目使用 PostgreSQL 持久化保存核心数据，线上部署时通过 `DATABASE_URL` 连接 Supabase 数据库。
...
│  └─ src/
│     ├─ index.js           # API 入口
│     ├─ db.js              # PostgreSQL 数据库和业务数据访问
│     ├─ insights.js        # 统计分析和行为建议
│     ├─ recommend.js       # 标签推荐
│     ├─ semantic.js        # 标签关系评分
│     ├─ deepseek.js        # DeepSeek 请求封装
│     └─ env.js             # 环境变量加载
├─ local-data/             # 旧版本地 SQLite 数据文件目录，线上版本不再使用
├─ scripts/                # 演示数据脚本
...
3. 新建一篇日志，展示 `POST /api/logs` 如何写入 PostgreSQL，并维护日志和标签的多对多关系。
